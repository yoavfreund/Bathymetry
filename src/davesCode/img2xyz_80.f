...
c
      ...
      rlt0=-80.738
      rln0=0.
      dln=0.016666666*mres
      dlt=0.
      rland=998.
      rdum=999.
      iproj=2
      write (*,904)
 904  format(' enter bounds for output grid - ',
...
c
      ihit=0
      write(*,905)
 905  format(' (0) - extract all grid points or',/,
     + ' (1) - only points constrained by measurements: ',$)
      read(*,*)ihit
c
...
c
      ...
      grav=(idat-itest)*scale
c
c  write out latitude, longitude and gravity anomaly (mgal) or depth
c
      ntot=ntot+1
      write(lu2,*)rlnt,rltt,grav
  150 continue
      endif
 200  continue
      call closio(-1)
      write(*,906)ntot
 906  format(' # of records in output file ',i8)
      stop
      end
c
