...
# 		echo "tile " $tileName $region
		dstFile="$dstDir/$tileName.grd"
		{
		grdcut $srcFile -G$dstFile $region 2> /dev/null

		((n_x=`grdinfo   $dstFile -C -M 2> /dev/null| awk '{print $10}'`)) 2> /dev/null
		((n_y=`grdinfo   $dstFile -C -M 2> /dev/null| awk '{print $11}'`))
		((n_nan=`grdinfo $dstFile -C -M 2> /dev/null| awk '{print $16}'`))
		((n_pings=`grdinfo $dstFile -C -M 2> /dev/null| awk '{print $10*$11-$16}'`))
		if [ "$n_pings" -le 0 ]; then rm $dstFile ; else echo "$dstFile $n_pings" ; fi
		} &
