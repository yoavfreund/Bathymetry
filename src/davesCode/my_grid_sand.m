...
%                       Hacked to handle gravity by JJ Becker
%
% WARNING!
% change DatabasesDir to the correct one for your machine
% latitudes must be between -72.006 and 72.006;
%
% Function MYGRID_SAND	Read bathymetry/gravity data from Sandwell Database
%
%      [image_data,vlat,vlon] = my_grid_sand(region,iopt)
%
% program to get bathymetry from topo_8.2.img.intel, which is the Smith and 
% Sandwell bathymetry (values are even numbered if interpolated, odd-numbered 
% if from a ship sounding) or gravity from grav.img.11.2.intel
%
% input:
...
%                (for iopt = 3) - matrix of sandwell gravity
%    
%		vlat - vector of latitudes associated with image_data
%      		vlon - vector of longitudes
% 
function  [image_data,vlat,vlon] = my_grid_sand(region,iopt)

if (iopt<=0 || iopt>=4)
	error(' Requested option, %d, is out of range (1-3)', iopt);return;
end

switch iopt
    case 3
        DatabasesDir = 'C:\Documents and Settings\jj\Desktop\SAR\grav\';
        DatabasesFile= 'grav.img.11.2.intel';
        scale = 10.;
    otherwise
        DatabasesDir = 'C:\Documents and Settings\jj\Desktop\jjgrids\';
        DatabasesFile= 'topo_8.2.img.intel';
        scale = 1.0;
    end
...
for i = 1:length(wlon);
	...
	% Open the data file
	fid = fopen([DatabasesDir DatabasesFile], 'r');
	if (fid < 0)
		errordlg(['Could not open database: ' DatabasesDir DatabasesFile],'Error');
	end
	...
	if (i>1)
		image_data = [image_data data]/scale;
        vlon       = [vlon-360 db_res*((iwlon(i)+1:ielon(i)+1)-0.5)];
	else
		image_data = data/scale;
...
% plot it up
% choose between ship track and data
if iopt ==2
  ...
  title('ship track soundings')
   ...
   if iopt==1
    ...
    title('Smith and Sandwell gravity...')
  end
end
