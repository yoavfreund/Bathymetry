...
c
      parameter (nltmx=17280,nlnmx=21600)
...
c
      write(*,901)
 901  format(' topography or gravity grid -',$)
      call inioc(lu1)
c
c get grid resolution
c
      write(*,902)
 902  format(' enter resolution of grid in minutes (1 or 2): ',$)
      read(*,*) mres
c
c  get the scale factor
c
      write(*,903)
 903  format(' enter scale factor (0.1-gravity) (1-topography): ',$)
      read(*,*) scale
c
c   open the file to be interpolated
c
      write(*,904)
 904  format(' points to be interpolated -',$)
      call inio(lui)
      write(*,905) 
 905  format(' lon field #, lat field #, total # (eg  2,3,8): ',$)
      read(*,*)lonfn,latfn,tfn
      write(*,906)
 906  format(' output file - ',$)
      call outio(luo)
c
...
c
      ...
      if(nlat.gt.nltmx.or.nlon.gt.nlnmx) then
        write(*,907)
 907    format('***** increase nltmx or nlnmx')
        stop
      endif
      do 101 i=1,nlat
      call rddiscb(lu1,iin,2*nlon,istat)
      if(istat.lt.0) then
 ...
 101  continue
      icall=1
 100  read(lui,*,end=999)(drec(ii),ii=1,tfn)
      rln = drec(lonfn)
...
c
  ...
  150   continue
      ...
      drec(tfn+1)=grav
      write(luo,990)(drec(ii),ii=1,tfn+1)
  990 format(20f12.4)
      go to 100
c
c  exit the program
